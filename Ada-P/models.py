...
class Model:
  ...
  def __init__(self):
    if FLAGS.mode == 'pretrain':
      self.images = tf.placeholder(tf.float32, [FLAGS.pretrain_batch_size, 84,84,3])
      self.labels = tf.placeholder(tf.int32, [None])
    else:
      self.images = tf.placeholder(tf.float32, [FLAGS.meta_batch_size, FLAGS.n_way*(FLAGS.k_shot+FLAGS.num_query), 84,84,3])
  ...
  def preprocess(self, inp, pretrain=False):
    ...
    if pretrain:
      ...
      images = tf.image.random_crop(images, [FLAGS.pretrain_batch_size,84,84,3])
    ...
    # normalisation 
    mean_dic = {'miniimagenet':[120.0112,114.617,102.7689], 'tieredimagenet':[120.0112,114.617,102.7689], 'cub':[103.432,103.432,99.4967]}
    std_dic = {'miniimagenet':[72.161,70.0569,73.7158], 'tieredimagenet':[72.161,70.0569,73.7158], 'cub':[58.3702,58.5375,62.7546]}
  ...
  def basic_conv_block(self, inp, num_filter_inp, num_filter, name):
    ...
    with tf.variable_scope(name) as scope:
      ...
      # some baselines
      # output = tf.nn.avg_pool(x, pooling_stride, pooling_stride, padding='SAME')
    return output
